...
def cifar10_model_fn(features, labels, mode, params):
  ...
  def network(input, *arg):
      out = tf.layers.conv2d(input, 6, kernel_size=[2, 2], strides=[2, 2], use_bias=False)
      out = tf.layers.batch_normalization(tf.nn.relu(out), training=True, fused=False)
      len = np.prod(out.get_shape().as_list()[1:])
      out = tf.layers.dense(tf.reshape(out, [-1, len]), _NUM_CLASSES)
      return out
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    initial_learning_rate = 0.01 * params['batch_size'] / 128
